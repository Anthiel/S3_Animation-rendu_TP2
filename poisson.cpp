...
bool Poisson::dans_voisinage( QVector3D p,int distanceVoisinage)
{
    return distanceVoisinage > position.distanceToPoint(p);
}
...
void Poisson::affiche(QOpenGLShaderProgram *program_poisson){
    ...
    if(vitesse.x() != 0 || vitesse.z() != 0){    //rotation sur le plan du sol sur 360°
        int signeAngle=1;
    ...
    if(vitesse.length() != 0){                  //rotation haut bas sur 180°
